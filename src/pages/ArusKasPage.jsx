...
export default function ArusKasPage() {
    ...
    const handleAddExpense = (data) => {
        addExpense(data.keterangan, data.nominal, data.tanggal)
        setShowModal(false)
    }
        ...
        <div className="fade-in">
            ...
            {filtered.length === 0 ? (
                <EmptyState
                    ...
                    message="Belum ada transaksi arus kas bulan ini. Transaksi pembayaran akan otomatis tercatat, atau tambahkan pengeluaran manual."
                    action={
                        <button className="btn btn-primary" onClick={() => setShowModal(true)}>
                            <Plus size={16} /> Tambah Pengeluaran
                        </button>
                            ...
                            <tbody>
                                {paginated.map((cf, i) => (
                                    <tr key={cf.id}>
                                        ...
                                        <td className="mono" style={{ fontWeight: 600 }}>{formatRupiah(cf.nominal)}</td>
                                        <td>
                                            {cf.tipe === 'masuk' ? (
