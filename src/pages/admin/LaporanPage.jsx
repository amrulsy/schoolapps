...
export default function LaporanPage() {
    const { bills, cashFlow, formatRupiah, categories, MONTHS, tahunAjaran } = useApp()
    ...
    // Monthly bar chart - Fix mapping and filter
    const monthlyData = useMemo(() => {
        return MONTHS.map((m, idx) => {
            // Mapping bulan berdasarkan tahun ajaran (Juli - Juni)
            // Juli (0) -> month index 6, ..., Desember (5) -> index 11, Januari (6) -> index 0, ..., Juni (11) -> index 5
    ...
    // Category pie chart - Use kategori_nama from backend
    const categoryData = useMemo(() => {
        return categories.map(cat => {
            const paid = bills.filter(b => (b.kategori_nama || b.kategori) === cat.nama && b.status === 'lunas' && b.tahun_ajaran === tahunAjaran).reduce((s, b) => s + Number(b.nominal), 0)
            ...
            <div className="grid-2">
                <div className="card">
                    <div className="card-header">
                        <h3>📊 Arus Kas Sekolah</h3>
                    </div>
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={monthlyData}>
                            <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" />
                            <XAxis dataKey="bulan" tick={{ fill: 'var(--text-secondary)', fontSize: 12 }} />
                            <YAxis tick={{ fill: 'var(--text-secondary)', fontSize: 12 }} tickFormatter={v => v >= 1000000 ? `${(v / 1000000).toFixed(1)}jt` : v >= 1000 ? `${(v / 1000).toFixed(0)}rb` : v} />
                            ...
                            {categories.map(cat => {
                                ...
                                return (
                                    <tr key={cat.id}>
                                        ...
                                        <td className="mono" style={{ color: 'var(--success-600)' }}>{formatRupiah(paidNominal)}</td>
                                        <td className="mono" style={{ color: 'var(--danger-500)' }}>{formatRupiah(unpaidNominal)}</td>
                                        <td>
