                            ...
                            <tbody>
                                {paginated.map((tx) => (
                                    <tr key={tx.id} style={{ opacity: (tx.status === 'void' || tx.status === 'voided') ? 0.6 : 1 }}>
                                        <td data-label="Invoice" className="mono" style={{ fontWeight: 700, color: 'var(--primary-600)' }}>{tx.invoice_no || tx.invoiceNo}</td>
                                        <td data-label="Tanggal">{new Date(tx.tanggal).toLocaleString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</td>
                                        <td data-label="Siswa" style={{ fontWeight: 500 }}>
