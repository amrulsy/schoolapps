...
function extractTables(sql) {
  ...
  const regex = /CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:`?\w+`?\.?)?`?(\w+)`?\s*\(([\s\S]*?)\)\s*(?:ENGINE|;)/gi;
  let match;
...
console.log("\n=== COLUMN DIFFERENCES INTABLES EXISTING IN BOTH ===");
for (let t in localTables) {
  if (hostingTables[t]) {
    ...
    for (let c in localCols) {
      if (!hostCols[c]) {
        ...
        // Maybe ignore type diffs first if we just want to find missing columns or note them
